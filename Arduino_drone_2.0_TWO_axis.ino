...
void setup() {
  ...
  R_F_prop.attach(6); //right front motor 
  R_B_prop.attach(7); //right back motor 
...
float yaw_adjustment = 300 * (input_YAW - 1490) / 500; 
float roll_adjustment = 300 * (input_ROLL - 1506) / 500; 
float pitch_adjustment = 300 * (input_PITCH - 1470) / 500; 
if(input_YAW > 1500){
  pwm_R_F += yaw_adjustment;
  pwm_L_B += yaw_adjustment;
}else if(input_YAW < 1400){
  pwm_R_B -= yaw_adjustment;
  pwm_L_F -= yaw_adjustment;
}
if(input_ROLL > 1510){
  Serial.println("Rolling +");
    Serial.println(roll_adjustment);
  pwm_R_F -= roll_adjustment;
  pwm_R_B -= roll_adjustment;
}else if(input_ROLL < 1504){
    Serial.println("Rolling -");
    Serial.println(roll_adjustment);
  pwm_L_F -= roll_adjustment;
  pwm_L_B -= roll_adjustment;
}
if(input_PITCH > 1472){
  pwm_R_B += pitch_adjustment;
  pwm_L_B += pitch_adjustment;
}else if(input_PITCH < 1468){
  pwm_R_F -= pitch_adjustment;
  pwm_L_F -= pitch_adjustment;
}
